...
# (ActiveEmotion, Mood, ConcreteEmotionalState, EmotionalAppraisalConfiguration),
# Cognitiv MIT commit f3aad875a77a3c7c522781e03acbb1944c3ab25c (multi-cause saturation),
# Alma.Net MIT commit 55b0475abdd077f145ed90575b435111c288454e (OCEAN/PAD mappings).
# Matrix-owned extensions: causal persistent-affect accounting and event idempotence.
...
class EmotionalImpulse:
    emotion_type: str; intensity: float; cause_id: str; target_id: Optional[str]=None; appraisal_channel: str="generic"
@dataclass
...
class AffectiveProfile:
    reactivity: float=1.; positive_reactivity: float=1.; negative_reactivity: float=1.; recovery_scale: float=1.; persistent_step: float=.05; mood_bias_strength: float=.15; mood_half_life: Optional[float]=None; ocean: Optional[OceanProfile]=None
@dataclass(frozen=True)
class AffectiveConfig:
    half_life_decay_constant: float=.5; emotion_influence_on_mood_factor: float=.3; mood_influence_on_emotion_factor: float=.3; minimum_mood_for_influence: float=.05; emotional_half_life_decay_time: float=15.; mood_half_life_decay_time: float=60.; extinction_threshold: float=.01
@dataclass
class EmotionState:
    emotions: Dict[str,float]=field(default_factory=dict); valence: float=0.; arousal: float=0.; dominance: float=0.; virtual_emotion_intensity: float=0.; mood_valence: float=0.; mood_arousal: float=0.
@dataclass
class PersistentAffect:
    trust: float=.5; attachment: float=0.; affection: float=0.; attraction: float=0.; resentment: float=0.; respect: float=.5; admiration: float=0.; aversion: float=0.
@dataclass
class _Contribution:
    emotion_type: str; intensity_at_t0: float; tick_t0: float; threshold: float; decay_multiplier: float; source_potential: float; persistent_intensity: float=0.
...
class AffectiveEngine:
    ...
    def apply_impulse(self,i):
        ...
        if raw<=th:self._update_dimensions();return prev is not None
        pot=self._determine_potential(i.emotion_type,raw)
        if pot<=th:self._update_dimensions();return prev is not None
        intensity=self._clamp(pot-th);new=_Contribution(i.emotion_type,intensity,self._time,th,self._fatima_decay_multiplier(d),raw,intensity);self._contributions[slot]=new;self._recompute_emotion(new.emotion_type)
    ...
    def reinforce(self,cause_id,appraisal_channel,target_id,potential):
        ...
        cur=self._decayed_contribution(c,self._time);p=self._clamp(potential);a=(cur+c.threshold)*10.;b=p*10.;pivot=max(a,b);r=min(1.,(pivot+math.log(math.exp(a-pivot)+math.exp(b-pivot)))/10.);ni=self._clamp(r-c.threshold);delta=ni-c.persistent_intensity
